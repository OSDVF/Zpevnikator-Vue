<template>
  <main class="container">
    <div class="p">
      <button
        class="btn btn-secondary pwa-d-none mb-2"
        onclick="checkState(false,true)"
      >Stáhnout offline</button>
      <button
        class="btn btn-light mb-2"
        onclick="downloadSongIndex(displaySongNumberInfo)"
      >Aktualizovat index písní</button>&ensp;
      ...
      <div class="list-group list-group-flush">
        ...
        <div class="list-group-item disabled" id="all_song_download">
          ...
          <span class="float-right">
            ...
            >Stáhnout všechny písně</button>&ensp;
    ...
    <div class="mb-4">
      <input
        type="file"
        class="custom-file-input w-25 position-absolute"
        id="inputFile"
        accept=".songs"
        multiple
      />
    ...
    <p>
      Verze Chrome/Safari:
      <span id="chromeVersion">? / Probíhá zjišťování</span>
      <br />
      <span class="pwa-d-none">
        Verdikt:
        <span id="detectionResult">Nevím jestli to bude fungovat, ale můžete to zkusit :D</span>
      </span>
      <span class="d-none pwa-d-initial">
        Verze stažené aplikace:
    ...
    <div class="list-group pwa-d-none" id="installManuals">
      <div class="expansion-panel pwa-d-none list-group-item manualInstallation">
        <a
          aria-controls="collapseInstallOne"
          aria-expanded="false"
          class="expansion-panel-toggler collapsed manualInstallation"
          data-toggle="collapse"
          href="#collapseInstallOne"
        >
        ...
        <div
          class="collapse manualInstallation"
          data-parent="#installManuals"
          id="collapseInstallOne"
        >
      ...
      <div class="expansion-panel pwa-d-none list-group-item iOSInstallation">
        <a
          aria-controls="collapseInstalliOS"
          aria-expanded="false"
          class="expansion-panel-toggler collapsed iOSInstallation"
          data-toggle="collapse"
          href="#collapseInstalliOS"
        >
      ...
      <div class="expansion-panel pwa-d-none list-group-item desktopManual">
        <a
          aria-controls="collapseInstallTwo"
          aria-expanded="false"
          class="expansion-panel-toggler collapsed desktopManual"
          data-toggle="collapse"
          href="#collapseInstallTwo"
        >
    ...
    <div
      ...
      id="manualInstallPrompt"
      tabindex="-1"
      role="dialog"
      aria-labelledby="manualInstallLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <P
              class="ios-d-block"
            >Na Apple zařízejích není automatická instalce možná, prosím postupujte podle návodu 🔽</P>
            <button
              class="btn btn-secondary"
              data-dismiss="modal"
              onclick="document.getElementById('navody').scrollIntoView()"
              aria-label="Jít na návod"
            >Jít na návod</button>
          </div>
        </div>
      </div>
...
export default {
  mounted() {
    ...
    $("#appVersion").html(process.env.VERSION);
    ...
    if (chromeVersion)
      ...
      $("#detectionResult")
        .html(
          "Aplikace není pro iOS a MacOS optimalizována. Ale měla by fungovat od <b>Safari 11</b>.<br>Na jiných prohlížečích na těchto systémech <b>nefunguje</b>"
        )
    ...
    if (chromeVersion >= 70) {
      $("#detectionResult")
        .html(
          "Jupí, u vás není pochybností, že by to nemohlo fungovat!<br>" +
            "Spustě automatickou instalaci tlačítkem."
        )
        .addClass("text-success");
    } else if (chromeVersion > 67) {
      $("#detectionResult")
        .html(
          "Je hodně pravděpodobné ze by to mohlo fungovat<br>" +
            "Po kliknutí na tlačítko a stažení stránek se zobrazí výzva k instalaci aplikace. Pokud ne, postupujte podle návodu:"
        )
        .addClass("text-success");
      ...
      $("#detectionResult").html(
        "Minimálně když zobrazíte v prohlížeči stránku chvaly.dorostmladez.cz, měla by být dostupná offline. Jestli vam půjde nainstalovat aplikaci je neznáme..."
      );
    } else {
      $("#detectionResult")
        .html("Vám to spíš nebude vůbec fungovat.")
    ...
    exportSong() {
      ...
      caches.open(songCache).then(function(cache) {
        return cache.keys().then(function(keys) {
          cache
            .matchAll("/api/getsong.php", { ignoreSearch: true })
                  ...
                  var dbPromise = new Promise(function(resolve, reject) {
                    songStore.get(id).onsuccess = function(event) {
                      if (event.target.result) {
                        ...
                        if (songInfo.info.offlineOnly)
                          delete songInfo.info.offlineOnly;
                        resolve();
                      } else {
                        console.error(
                          'Could not find info about song "' + id + '"'
                        );
                        reject();
                      }
                    };
          ...
          r.onload = function(e) {
            try {
                ...
                if (songsInfo.length)
                  for (let j = 0; j < songsInfo.length; j++) {
                    ...
                    if (
                      ...
                      SongDB.updateSong(null, songInfo.info, function(
                        changedInfo
                      ) {
                  ...
                  exceptions.push(
                    new SyntaxError("Song list JSON badly formatted", file.name)
                  );
                }
              ...
              if (i == curFiles.length - 1) {
                importButton.insertAdjacentHTML(
                  "afterend",
                  '<i class="ml-4 text-warning material-icons">warning</i><span class="text-warning"> (byly nalezeny chyby)</span>'
                );
                SongDB.informAboutChanges(changedSongs, newSongs);
                $("#inputFile").val("");
              }
