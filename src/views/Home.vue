<template>
  <main class="homeWrapper">
    ...
    <div class="mainpage container">  
      ...
      <p
        class="font-weight-light typography-title mb-2"
      >Písně 🎵 | Akordy 🎼 | Transpozice 📈 | Konverze do Wordu 📑</p>
      ...
      <button class="btn btn-transparent text-white" @click="addStar">
        ...
        <span
          id="starNum"
          data-toggle="tooltip"
          title="Tolik lidí už přidalo hvězdičku 😎"
        >0</span>&ensp;
        <span
          data-toggle="tooltip"
          title="Líbí se mi to a chci přidat 🌟"
          style="background:#ffffff57;padding:15px 17px 15px 13px;margin:0 -17px 0 4px;"
        >+1</span>
      </button>
      <hr />
      <p
        class="typography-subheading"
      >To vše teď umí dorostomládežový zpěvník. Stačí jen vzít něco do ruky 🎹🎸🎤 a začít!</p>
      <p class="typography-caption">
        Stabilní verze {{version}} (aktualizace {{lastUpdate}})
        <br />Pokud chcete vidět nejnovější funkce, podívejte se na
        <a
          href="https://alpha.dorostmladez.cz"
          class="text-visible"
        >Experimentální verzi</a>
      </p>
      <p class="typography-caption">
        <a
          href="https://dorostmladez.cz/chvaly"
          target="_blank"
          class="text-visible"
        >Stará verze zpěvníku zde (pro staré prohlížeče)</a>
      </p>
    </div>
  </main>
</template>
...
export default {
  ...
  created() {
    ...
    this.lastUpdate = process.env.VUE_APP_LAST_UPDATE;
  },
  mounted() {
    globalManager.resourcesReady(this, () => {
      ...
      if (window.innerWidth >= 768) {
        $(".tricker").append(
          "<h2>Nebo prostě k jakémukoliv zpěvníkování 🤩</h2>"
        );
        duration = 2500;
      }
      var tricker = $(".tricker");
  ...
  methods: {
    addStar: function() {
      localStorage.getItem(settingsPrefix + "likeCount")
        ? localStorage[settingsPrefix + "likeCount"]++
        : (localStorage[settingsPrefix + "likeCount"] = 1);
