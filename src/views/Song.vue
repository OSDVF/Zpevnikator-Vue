<template>
  <main class="songPage">
    <div class="float-md-right">
      <div class="text-secondary font-weight-bold mr-lg-5" v-if='transpositionInfo!=0'>Transponováno o {{transpositionInfo>0?('+'+transpositionInfo):transpositionInfo}}</div>
    </div>
    ...
    <div :class="'flex-wrap justify-content-start  justify-content-sm-center justify-content-lg-start d-'+(lastSongResponse==null?'none':'flex')">
      <div class="mr-3">
        <button class="mb-2 btn btn-transparent" data-target="#songInfo" data-toggle="collapse"><i class="material-icons">info</i>&ensp;Informace<br></button>
        <div class="collapse" id='songInfo'>
          <div class="list-group list-group-flush">
            ...
            <span class="list-group-item" v-if='specialStateIcon.length' v-html="specialStateIcon+'&ensp;'+specialStateLabel"></span>
      ...
      <div>
        <button class="mb-2 btn btn-transparent" data-target="#songInfo" data-toggle="collapse"><i class="material-icons">arrow_forward</i>&ensp;Akce<br></button>
        <div class="collapse" id='songInfo'>
          <div class="list-group list-group-flush">
            ...
            <div id="actions">
              <button id="edit" class="list-group-item-action list-group-item" @click="edit"><i class="material-icons">edit</i>&ensp;Upravit&ensp;<span class="text-warning" v-if="!specialStateIcon.length"><i class="material-icons">warning</i> Nebyla přidána vámi</span></button>
...
export default {
	...
	computed: {
		...
		specialStateIcon() {
			var specialStateIcon = "";
			if (this.songInfo.status) specialStateIcon += SongProcessing.statusToIcon(this.songInfo.status);
			if (this.songInfo.offlineOnly) specialStateIcon += '<i class="text-secondary material-icons">cloud_off</i>';
			return specialStateIcon;;
		},
		specialStateLabel() {
			var specialStateLabel = "";
			if (this.songInfo.status) {
				specialStateLabel += SongProcessing.statusToDescription(this.songInfo.status);
			}
			if (this.songInfo.offlineOnly) {
				if (this.songInfo.status) specialStateLabel += ", ";
				specialStateLabel += "Jen v offline databázi";
			}
			return specialStateLabel;
		}
	},
	mounted() {
	...
	methods: {
		...
		share() {
			if (typeof navigator.share != "undefined") navigator.share({ title: this.$store.state.title, url: location.href });
				...
				UIHelpers.Message("Adresa je ve vaší schránce", null, 1200);
			}
		},
		changeDisplayedSong(url, forceFetch) {
			...
			SongDB.get(url, async inf => {
				...
				if (!inf.status) inf.status = false;
				if (!inf.offlineOnly) inf.offlineOnly = false;
		...
		applyTransposition(amount) {
			...
			this.transpositionInfo = amount ? amount : 0;
		},
		alterAll() {
			const _self = this;
			$(".chord").each(function() {
